<template>
  <div v-if="titles.length > 0" class="mt-12 bg-card p-6 rounded-xl border border-gray-800">
    <h2 class="text-2xl font-bold mb-4 tracking-tight border-l-4 border-brand pl-3">Похожие произведения</h2>
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
      <a v-for="title in titles" :key="title.id" :href="`/watch/${title.id}/`" class="group block">
        <div
          ...
          <div v-else class="w-full h-full flex items-center justify-center text-gray-700 font-bold text-sm">Нет постера
          </div>
