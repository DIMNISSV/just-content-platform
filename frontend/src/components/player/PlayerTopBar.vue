...
const rateTrack = (score) => {
  ...
  showTrackRating.value = false; // Закрываем после клика
};
</script>

<template>
  <div
      class="p-4 bg-gradient-to-b from-black/80 to-transparent flex flex-col sm:flex-row gap-4 items-start sm:items-center pointer-events-auto">

    <!-- Audio Selection -->
    <div v-if="currentGroupAudios.length > 0"
         class="flex items-center bg-gray-900/80 border border-gray-700 rounded overflow-hidden shadow-lg backdrop-blur">
    ...
    <!-- Version Selection & Rating -->
    <div
        class="flex items-center bg-gray-900/80 border border-gray-700 rounded overflow-hidden shadow-lg backdrop-blur relative">
      ...
      <!-- Звезда рейтинга -->
      <div class="border-l border-gray-700 px-2 flex items-center h-full">
        <button
            @click="toggleRatingMenu"
            class="text-yellow-500 hover:scale-125 transition-transform py-1.5"
