  ...
  <div class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
    <div
      ...
      <div class="p-6 border-b border-gray-800">
        <h3 class="text-xl font-bold">Сопоставление для: {{ targetName }}</h3>
        <p class="text-xs text-gray-500 mt-1">Файл: {{ file.original_name }}</p>
      </div>

      <div class="flex-grow overflow-y-auto p-6 space-y-4">
        <div v-if="dropType === 'new_group'" class="mb-4">
          <label class="block text-xs font-bold text-gray-500 mb-1">Релиз-группа видео (опционально)</label>
          <input v-model="groupAuthor" type="text" placeholder="например: YIFY, Remux"
        ...
        <div v-if="dropType === 'new_group'"
          ...
          Информация: Вы должны выбрать ровно один <strong>видеопоток</strong> для создания новой версии.
        </div>
          ...
          <div class="flex-grow">
            <div class="flex items-center gap-2">
              <span class="text-[10px] font-black uppercase px-1.5 py-0.5 rounded"
                ...
                {{ s.codec_type === 'video' ? 'видео' : 'аудио' }}
              </span>
            ...
            <div v-if="s.selected && !s.is_video" class="mt-3 flex gap-4">
              <div class="flex-1">
                <label class="block text-[10px] uppercase font-bold text-gray-500 mb-1">Код языка</label>
              ...
              <div class="flex-1">
                <label class="block text-[10px] uppercase font-bold text-gray-500 mb-1">Автор / Студия</label>
                <input v-model="s.custom_author" type="text" placeholder="LostFilm"
                       class="w-full bg-black border border-gray-700 rounded p-1.5 text-xs text-white">
              </div>
              <div class="flex-1">
                <label class="block text-[10px] uppercase font-bold text-gray-500 mb-1">Смещение (мс)</label>
