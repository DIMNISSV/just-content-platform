    ...
    try:
        with transaction.atomic():
            ...
            if season_num is not None and episode_num is not None:
                episode = Episode.objects.filter(title=title, season_number=season_num,
                                                 episode_number=episode_num).first()
                if not episode:
                    if plugin.allow_title_creation:
                        episode = Episode(
                            title=title,
        ...
        logger.error(f"Error saving registry entry or applying metadata: {str(e)}")
        return {"error": f"Internal server error during payload processing: {str(e)}"}, 500
